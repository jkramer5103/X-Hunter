...
# join_room und leave_room importieren
...
SEEKERS_CHAT_ROOM = 'seekers_chat_room' # Name für den Chat-Raum
...
def handle_connect():
    ...
    username = session["username"]; sid = request.sid
    ...
    # Sucher zum Chat-Raum hinzufügen
    if username != game_state["mr_x"]:
        join_room(SEEKERS_CHAT_ROOM, sid=sid)
        print(f"Seeker {username} joined room {SEEKERS_CHAT_ROOM}")
    ...
    for player, data in game_state["players"].items():
        ...
        if is_invisible and player != username: continue
        if player != game_state["mr_x"] and loc: current_locations[player] = {"lat": loc["lat"], "lon": loc["lon"]}
        elif player == game_state["mr_x"] and game_state["mr_x_last_known_location"]:
             current_locations[player] = { "lat": game_state["mr_x_last_known_location"]["lat"], "lon": game_state["mr_x_last_known_location"]["lon"] }
...
def handle_disconnect():
    ...
    if disconnecting_user:
        ...
        if game_state["active"]:
            # Sucher aus Chat-Raum entfernen
            if disconnecting_user != game_state["mr_x"]:
                leave_room(SEEKERS_CHAT_ROOM, sid=sid)
                print(f"Seeker {disconnecting_user} left room {SEEKERS_CHAT_ROOM}")
...
def handle_activate_invisibility():
    ...
    duration = game_state["seeker_invisibility_duration_seconds"]; expiration_time = time.time() + duration
...
def handle_send_chat_message(data):
    """Empfängt Chat-Nachricht von einem Sucher und sendet sie an andere Sucher."""
    if "username" not in session or not game_state["active"]: return
    username = session["username"]
    if username == game_state["mr_x"]: print(f"Mr. X ({username}) tried to send chat message. Ignored."); return
    if username not in game_state["players"]: print(f"User {username} tried to send chat message but is not in game. Ignored."); return

    message_text = data.get('message', '').strip()
    if not message_text: return
    max_len = 200
    if len(message_text) > max_len: message_text = message_text[:max_len] + "..."

    print(f"Chat message from {username} to room {SEEKERS_CHAT_ROOM}: {message_text}")
    # Sende an alle im 'seekers' Raum, *einschließlich* des Senders selbst
    socketio.emit("new_chat_message", {
        "username": username,
        "message": message_text
    }, room=SEEKERS_CHAT_ROOM) # Sende an alle im Raum
...
if __name__ == "__main__":
    print("Starting Flask-SocketIO server for Mr. X Game (with chat)...")
